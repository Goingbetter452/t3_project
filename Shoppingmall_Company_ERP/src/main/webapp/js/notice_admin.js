            ...
            '<div class="notice-header">' +
                ...
                '<div class="notice-meta">' +
                    ...
                    '<div class="notice-actions">' +
                        '<button class="edit-notice-btn" onclick="editNotice(' + notice.noticeId + ')">수정</button>' +
...
function editNotice(noticeId) {
    ...
    if (!notice) {
        showAlert('공지사항을 찾을 수 없습니다.', 'error');
        return;
    }

    // JSP 팝업창 열기
    const popup = window.open(
        'notice_edit_popup.jsp?noticeId=' + noticeId, 
        'editNotice', 
        'width=600,height=500,scrollbars=yes,resizable=yes,location=no,menubar=no,toolbar=no'
    );
    
    // 팝업창이 차단되었는지 확인
    if (!popup) {
        showAlert('팝업 차단이 설정되어 있습니다. 팝업을 허용해주세요.', 'warning');
        return;
    }
}

// 팝업에서 호출될 수정 저장 함수
function updateNoticeFromPopup(noticeId, title, content, popupWindow) {
    fetch('GroupwareServlet', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
